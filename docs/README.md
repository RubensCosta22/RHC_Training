...
RHC Training follows the **RHC Tech Spec-Driven Development Standard v1.2**.
...
### R0
Use `docs/templates/lightweight-change-record.md` only when runtime behavior is provably unchanged.
...
10. Merge, deploy and perform required production verification.

Material requirement or risk discoveries return the Spec to review.

## Pull requests

`.github/pull_request_template.md` is the default evidence/gate record. A successful merge or deployment does not prove that a change is Done or healthy in production.
