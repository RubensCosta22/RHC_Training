...
export default function Login() {
  ...
  async function handlePasswordReset() {
    ...
    try {
      ...
      setMessage('Se o e-mail estiver cadastrado, você receberá um link temporário para criar uma nova senha.')
    } catch (error) {
    ...
    <main className="grid min-h-screen bg-[#08090a] lg:grid-cols-[minmax(0,1.45fr)_minmax(420px,.55fr)]">
      <section className="relative min-h-[46vh] overflow-hidden lg:min-h-screen">
        ...
        <div className="relative flex min-h-[46vh] flex-col justify-between p-6 sm:p-10 lg:min-h-screen lg:p-12 xl:p-16">
          ...
          <div className="max-w-2xl"><p className="mb-4 text-[11px] font-bold uppercase tracking-[.16em] text-[#c8ff3d]">Consistência muda tudo</p><h1 className="text-[clamp(3rem,6vw,6.5rem)] font-[780] leading-[.88] tracking-[-.075em] text-white">Treine.<br/>Evolua.<br/>Repita.</h1><p className="mt-5 hidden max-w-md text-base text-white/65 sm:block">Sua rotina, seu histórico e sua evolução em um único lugar.</p></div>
        </div>
      </section>

      <section className="flex items-center px-6 py-10 sm:px-10 lg:px-12 xl:px-16"><div className="mx-auto w-full max-w-md">
        <div className="mb-8"><p className="rhc-kicker mb-3">Sua conta</p><h2 className="text-3xl font-[720] tracking-[-.04em]">{mode==='login'?'Bem-vindo de volta.':'Comece sua jornada.'}</h2><p className="mt-2 text-sm text-[#92979f]">Entre para continuar seu próximo treino.</p></div>
