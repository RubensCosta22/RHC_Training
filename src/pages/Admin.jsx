...
export default function Admin() {
  ...
  return <div>
    <PageHeader eyebrow="Administração" title="Central RHC" subtitle="Pessoas, performance e acesso em uma única visão." action={<button onClick={logout} type="button" aria-label="Sair" className="grid h-11 w-11 place-items-center rounded-full border border-[#272a2f] text-[#92979f] transition hover:border-[#ff6b70]/40 hover:text-[#ff6b70]"><LogOut size={18}/></button>} />

    <section className="mb-14 grid gap-8 border-y border-[#272a2f] py-8 sm:grid-cols-[1fr_auto] sm:items-end">
      <div><p className="rhc-kicker mb-3">Visão geral</p><div className="flex items-baseline gap-4"><span className="tabular-nums text-7xl font-[760] tracking-[-.065em]">{profiles.length}</span><span className="max-w-32 text-sm leading-snug text-[#62676f]">perfis sob sua supervisão</span></div></div>
    ...
    {showCreate && <form className="mb-8 grid gap-4 border-b border-[#272a2f] bg-[#0d0f11] p-5 sm:grid-cols-2" onSubmit={createProfile}>
      <div className="sm:col-span-2"><h3 className="text-lg font-black">Adicionar perfil</h3><p className="text-sm text-slate-400">O titular receberá acesso ao entrar com o e-mail associado. O V2 usa vínculo individual 1:1 entre conta e perfil.</p></div>
      ...
      <label className="text-sm text-slate-300">Gênero<select value={newProfile.gender} onChange={(e)=>setNewProfile({...newProfile,gender:e.target.value})}><option value="mulher">Mulher</option><option value="homem">Homem</option><option value="outro">Outro</option></select></label>
