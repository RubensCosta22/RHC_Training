...
export async function getExerciseRecords(profileId, exerciseNames = []) {
  ...
  if (exerciseNames.length) query = query.in('exercise_name', exerciseNames)
...
export async function saveWorkoutSession({ profileId, workoutType, date, gymName, durationMinutes, notes, exercises }) {
  const { data: userData, error: userError } = await supabase.auth.getUser()
  if (userError) throw userError
  const userId = userData.user?.id
  if (!userId) throw new Error('Faça login novamente para salvar o treino.')
  ...
  const completionPercentage = cleanExercises.length ? Math.round((completedCount / cleanExercises.length) * 100) : 0
  ...
  const exercisesPayload = cleanExercises.map((item) => ({ ...item, session_id: session.id }))
  const { error: exercisesError } = await supabase.from('workout_exercises').insert(exercisesPayload)
  if (exercisesError) throw exercisesError

  await updateExerciseRecords({ userId, profileId, date, exercises: cleanExercises })
  return session
}

async function updateExerciseRecords({ userId, profileId, date, exercises }) {
  const existing = await getExerciseRecords(profileId, exercises.map((item) => item.exercise_name))
  const payload = exercises
    ...
    .map((item) => {
      ...
      return {
        ...
        best_weight: Math.max(Number(item.weight || 0), Number(previous?.best_weight || 0)),
        last_date: date,
  ...
  const { error } = await supabase
    .from('exercise_records')
    .upsert(payload, { onConflict: 'user_id,profile_id,exercise_name' })
...
export async function getWorkoutSessions(profileId, filters = {}) {
  ...
  if (filters.type) query = query.eq('workout_type', filters.type)
  if (filters.gymName) query = query.ilike('gym_name', `%${sanitizeText(filters.gymName, 80)}%`)
...
export async function getDashboardSummary(profileId) {
  ...
  const weekCount = normalized.filter((s) => new Date(s.date) >= startOfWeek).length
  const monthCount = normalized.filter((s) => new Date(s.date) >= startOfMonth).length
  const totalVolume = normalized.reduce((acc, s) => acc + Number(s.total_volume || 0), 0)
  const streak = calculateStreak(normalized.map((s) => s.date))
  const bestStreak = calculateBestStreak(normalized.map((s) => s.date))
...
function calculateBestStreak(dates) {
  ...
  for (let i = 1; i < unique.length; i += 1) {
    ...
    const actual = new Date(unique[i])
...
export async function getProgressData(profileId) {
  ...
  return { sessions: sessions.reverse(), measurements: measurements || [], exercises }
}
